...
st.set_page_config(page_title="Dashboard RH - Análise de Colaboradores", layout="wide", page_icon="📊")
...
CIDADES_MAPA = {
    'SBCD - AME CRI ZN': {'cidade': 'São Paulo', 'estado': 'SP', 'lat': -23.5505, 'lon': -46.6333},
    'SBCD - ANDRADINA AME': {'cidade': 'Andradina', 'estado': 'SP', 'lat': -20.8958, 'lon': -51.3786},
    'SBCD - CANDIDO FERRAZ (HRSCF)': {'cidade': 'São Raimundo Nonato', 'estado': 'PI', 'lat': -9.0115, 'lon': -42.6983},
    'SBCD - CENTRAL DE EXAMES DE PICOS': {'cidade': 'Picos', 'estado': 'PI', 'lat': -7.0767, 'lon': -41.4669},
    'SBCD - CENTRAL DE EXAMES VALENÇA': {'cidade': 'Valença do Piauí', 'estado': 'PI', 'lat': -6.4058, 'lon': -41.7461},
    'SBCD - CER II SAO JOAO DO PIAUI': {'cidade': 'São João do Piauí', 'estado': 'PI', 'lat': -8.3589, 'lon': -42.2447},
    'SBCD - CETEA - PI': {'cidade': 'Teresina', 'estado': 'PI', 'lat': -5.0919, 'lon': -42.8034},
    'SBCD - CUBATÃO - APS': {'cidade': 'Cubatão', 'estado': 'SP', 'lat': -23.8950, 'lon': -46.4253},
    'SBCD - GARÇA': {'cidade': 'Garça', 'estado': 'SP', 'lat': -22.2111, 'lon': -49.6522},
    'SBCD - HMC - CUBATÃO - NOVO': {'cidade': 'Cubatão', 'estado': 'SP', 'lat': -23.8950, 'lon': -46.4253},
    'SBCD - HOSP ITURAMA': {'cidade': 'Iturama', 'estado': 'MG', 'lat': -19.7292, 'lon': -50.1950},
    'SBCD - HOSP JUSTINO LUZ': {'cidade': 'Picos', 'estado': 'PI', 'lat': -7.0767, 'lon': -41.4669},
    'SBCD - HOSP PICOS': {'cidade': 'Picos', 'estado': 'PI', 'lat': -7.0767, 'lon': -41.4669},
    'SBCD - HOSP REG EUSTAQUIO PORTELA': {'cidade': 'São Paulo', 'estado': 'SP', 'lat': -23.5505, 'lon': -46.6333},
    'SBCD - ITU RT': {'cidade': 'Itu', 'estado': 'SP', 'lat': -23.2642, 'lon': -47.2997},
    'SBCD - MOCAMBINHO': {'cidade': 'São Paulo', 'estado': 'SP', 'lat': -23.5505, 'lon': -46.6333},
    'SBCD - PAI ZN': {'cidade': 'São Paulo', 'estado': 'SP', 'lat': -23.5505, 'lon': -46.6333},
    'SBCD - PROMISSÃO AME': {'cidade': 'Promissão', 'estado': 'SP', 'lat': -21.5372, 'lon': -49.8581},
    'SBCD - PRONTO ATENDIMENTO SAO VICENTE DE PAULO': {'cidade': 'São Paulo', 'estado': 'SP', 'lat': -23.5505, 'lon': -46.6333},
    'SBCD - REDE ASSIST. NORTE-SP': {'cidade': 'São Paulo', 'estado': 'SP', 'lat': -23.5505, 'lon': -46.6333},
    'SBCD - SEDE CORPORATIVA TERESINA': {'cidade': 'Teresina', 'estado': 'PI', 'lat': -5.0919, 'lon': -42.8034},
    'SBCD - UPA DE SÃO RAIMUNDO NONATO': {'cidade': 'São Raimundo Nonato', 'estado': 'PI', 'lat': -9.0115, 'lon': -42.6983},
    'SBCD - UPA DR THELMO JACAREÍ': {'cidade': 'Jacareí', 'estado': 'SP', 'lat': -23.3054, 'lon': -45.9658},
    'SBCD - UPA ITU': {'cidade': 'Itu', 'estado': 'SP', 'lat': -23.2642, 'lon': -47.2997},
    'SBCD - UPA PICOS': {'cidade': 'Picos', 'estado': 'PI', 'lat': -7.0767, 'lon': -41.4669}
}
...
def classificar_linha_cuidado(nome_fantasia, centro_custo):
    ...
    if any(palavra in texto for palavra in ['AME', 'UPA', 'CER']):
        ...
        return 'APS'
    elif any(palavra in texto for palavra in ['HOSP', 'HMC', 'HRSCF', 'PRONTO ATENDIMENTO']):
        return 'HOSPITAIS'
...
def load_and_process_data():
    ...
    if not os.path.exists(caminho_arquivo):
        ...
        st.info("Certifique-se de que o arquivo 'oris.xlsx' está na mesma pasta do arquivo dashboard_rh.py")
        st.stop()
    ...
    # Converter datas
    date_columns = ['Dt Admissão', 'Dt Rescisão', 'Dt Nascimento', 'Dt Início Cargo', 
                   'Dt Início Centro de Custo', 'Dt Início Empresa', 'Dt Início Escala',
                   'Dt Início Local de Trabalho', 'Dt Início Situação', 'Dt Início Tipo Funcionário']
    ...
    df['Linha de Cuidado'] = df.apply(lambda x: classificar_linha_cuidado(
    ...
    df['Demitido 45 dias'] = (df['Tempo Permanência (dias)'] <= 45) & (df['Demitido'] == 'Sim')
    df['Demitido 90 dias'] = ((df['Tempo Permanência (dias)'] > 45) & 
...
def calcular_substituicoes(df, data_inicio, data_fim):
    ...
    demissoes = df[(df['Demitido'] == 'Sim') & 
                   (df['Dt Rescisão'] >= data_inicio) & 
                   (df['Dt Rescisão'] <= data_fim)].copy()
    ...
    for idx, demissao in demissoes.iterrows():
        ...
        if len(candidatos) > 0:
            ...
            dias_substituicao = (candidato['Dt Admissão'] - demissao['Dt Rescisão']).days
...
centros_custo_disponiveis = ['TODOS'] + sorted(df_temp['Centro custo'].dropna().unique().tolist())
centro_custo_selecionado = st.sidebar.selectbox("🏥 Centro de Custo:", centros_custo_disponiveis)
...
with col4:
    taxa_sub = (len(df_substituicoes) / len(demissoes_periodo) * 100) if len(demissoes_periodo) > 0 else 0
...
tab1, tab2, tab3 = st.tabs(["📈 Movimentações", "🚪 Motivos de Desligamento", "⏱️ Análise de Permanência"])
    ...
    with subtab1:
        ...
        with col_a1:
            ...
            if not adm_nivel.empty:
                fig_adm_nivel = px.pie(adm_nivel, values='Quantidade', names='Nivel',
                                       color_discrete_sequence=px.colors.qualitative.Set2,
                                       hole=0.4)
                fig_adm_nivel.update_traces(textposition='inside', textinfo='percent+value')
        ...
        with col_a2:
            ...
            adm_linha = admissoes_periodo['Linha de Cuidado'].value_counts().reset_index()
            adm_linha.columns = ['Linha de Cuidado', 'Quantidade']
            
            if not adm_linha.empty:
                fig_adm_linha = px.pie(adm_linha, values='Quantidade', names='Linha de Cuidado',
                                       color_discrete_sequence=px.colors.qualitative.Pastel)
                fig_adm_linha.update_traces(textposition='inside', textinfo='percent+value')
        ...
        adm_mensal = admissoes_periodo.groupby('Mês/Ano Admissão').size().reset_index(name='Quantidade')
        adm_mensal = adm_mensal.sort_values('Mês/Ano Admissão')
        
        if not adm_mensal.empty:
            fig_adm_mensal = px.line(adm_mensal, x='Mês/Ano Admissão', y='Quantidade',
    ...
    with subtab2:
        ...
        with col_d1:
            ...
            if not dem_nivel.empty:
                fig_dem_nivel = px.pie(dem_nivel, values='Quantidade', names='Nivel',
                                       color_discrete_sequence=px.colors.qualitative.Set3,
                                       hole=0.4)
                fig_dem_nivel.update_traces(textposition='inside', textinfo='percent+value')
        ...
        with col_d2:
            st.markdown("**Por Linha de Cuidado**")
            dem_linha = demissoes_periodo['Linha de Cuidado'].value_counts().reset_index()
            dem_linha.columns = ['Linha de Cuidado', 'Quantidade']
            
            if not dem_linha.empty:
                fig_dem_linha = px.pie(dem_linha, values='Quantidade', names='Linha de Cuidado',
                                       color_discrete_sequence=px.colors.qualitative.Vivid)
                fig_dem_linha.update_traces(textposition='inside', textinfo='percent+value')
        ...
        dem_mensal = demissoes_periodo.groupby('Mês/Ano Rescisão').size().reset_index(name='Quantidade')
        dem_mensal = dem_mensal.sort_values('Mês/Ano Rescisão')
        
        if not dem_mensal.empty:
            fig_dem_mensal = px.line(dem_mensal, x='Mês/Ano Rescisão', y='Quantidade',
                                     markers=True, line_shape='spline')
    ...
    with subtab3:
        ...
        if len(df_substituicoes) > 0:
            ...
            with col_s1:
                ...
                st.metric("⏱️ Tempo Médio de Substituição", 
            ...
            fig_sub = px.pie(dados_sub, values='Quantidade', names='Status',
            ...
            st.dataframe(df_substituicoes, use_container_width=True)
        else:
...
with tab2:
    ...
    if len(df_desl) == 0:
        ...
        fig_geral = px.bar(motivos_geral, x='Quantidade', y='Tipo Rescisão', orientation='h',
        ...
        for linha in sorted(linhas_disponiveis):
            with st.expander(f"📋 {linha}", expanded=True):
                ...
                motivos_linha = df_linha['Tipo Rescisão'].value_counts().reset_index()
                motivos_linha.columns = ['Tipo Rescisão', 'Quantidade']
                
                if not motivos_linha.empty:
                    altura_linha = max(300, len(motivos_linha) * 30)
                    
                    fig_linha = px.bar(motivos_linha, x='Quantidade', y='Tipo Rescisão', orientation='h',
                                     color='Quantidade', color_continuous_scale='Teal',
                                     text='Quantidade')
                    fig_linha.update_traces(textposition='outside', textfont_size=11)
...
with tab3:
    ...
    if len(df_perm) == 0:
        ...
        with col_p2:
            perc_45 = (demitidos_45 / total_demitidos * 100) if total_demitidos > 0 else 0
            st.metric("📉 Demitidos até 45 dias", f"{perc_45:.1f}%", 
                     help=f"{demitidos_45} colaboradores")
        
        with col_p3:
            perc_90 = (demitidos_90 / total_demitidos * 100) if total_demitidos > 0 else 0
            st.metric("📉 Demitidos 45-90 dias", f"{perc_90:.1f}%",
                     help=f"{demitidos_90} colaboradores")
        
        with col_p4:
            demitidos_apos_90 = total_demitidos - demitidos_45 - demitidos_90
            perc_apos_90 = (demitidos_apos_90 / total_demitidos * 100) if total_demitidos > 0 else 0
            st.metric("✅ Permaneceram >90 dias", f"{perc_apos_90:.1f}%",
        ...
        fig_exp = px.bar(dados_experiencia, x='Período', y='Quantidade',
        ...
        with col_pn1:
            ...
            perm_nivel = df_perm.groupby('Nivel')['Tempo Permanência (dias)'].mean().reset_index()
            perm_nivel.columns = ['Nivel', 'Permanência Média (dias)']
            perm_nivel = perm_nivel.sort_values('Permanência Média (dias)', ascending=False)
            
            if not perm_nivel.empty:
                fig_perm_nivel = px.bar(perm_nivel, x='Permanência Média (dias)', y='Nivel',
                                       orientation='h', color='Permanência Média (dias)',
                                       color_continuous_scale='Blues')
                fig_perm_nivel.update_layout(showlegend=False)
                st.plotly_chart(fig_perm_nivel, use_container_width=True)
        
        with col_pn2:
            st.subheader("📋 Permanência Média por Linha de Cuidado")
            perm_linha = df_perm.groupby('Linha de Cuidado')['Tempo Permanência (dias)'].mean().reset_index()
            perm_linha.columns = ['Linha de Cuidado', 'Permanência Média (dias)']
            perm_linha = perm_linha.sort_values('Permanência Média (dias)', ascending=False)
            
            if not perm_linha.empty:
                fig_perm_linha = px.bar(perm_linha, x='Permanência Média (dias)', y='Linha de Cuidado',
                                       orientation='h', color='Permanência Média (dias)',
                                       color_continuous_scale='Greens')
                fig_perm_linha.update_layout(showlegend=False)
        ...
        for mes in meses_periodo:
            ...
            perm_mes = df_perm[df_perm['Mês/Ano Rescisão'] == mes_str]['Tempo Permanência (dias)'].mean()
            
            dados_evolucao.append({
                'Mês': mes_str,
                'Demissões': demissoes_mes,
                'Permanência Média (dias)': perm_mes if not pd.isna(perm_mes) else 0
            })
        ...
        if not df_evolucao.empty:
            ...
            fig_evolucao.update_layout(
                ...
                yaxis2=dict(title='Permanência Média (dias)', side='right', overlaying='y'),
                hovermode='x unified',
